...
# Update selected fields only (faster)
post = Post.objects.get(id=1)
post.title = "Title v2"
...
# bulk_update() — update many Python objects at once
...
# Delete using exclude
Post.objects.exclude(author="admin").delete()

# Delete using list of IDs
Post.objects.filter(id__in=[1, 2, 3]).delete()

# Delete with related lookup
Post.objects.filter(author__id=10).delete()
